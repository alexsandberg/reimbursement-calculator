...
def get_highest_full_day_rate(projectSet: ProjectSet, project_numbers: List[int]) -> int:
    highest_rate = 0
...
def get_highest_travel_day_rate(projectSet: ProjectSet, project_numbers: List[int]) -> int:
    highest_rate = 0
    for project_number in project_numbers:
        project = projectSet.get_project_by_number(project_number)
    ...
    for date, projects in projects_by_day.items():
        ...
        if number_of_projects == 0:
            ...
            if yesterday in projects_by_day and len(projects_by_day[yesterday]) != 0:
                ...
                rate_by_day[yesterday] = get_highest_travel_day_rate(projectSet, projects_by_day[yesterday])
            ...
            if tomorrow in projects_by_day and len(projects_by_day[tomorrow]) != 0:
                # there could be more than one project ending on same day
                rate_by_day[tomorrow] = get_highest_travel_day_rate(projectSet, projects_by_day[tomorrow])
